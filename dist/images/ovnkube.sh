...
# Enable cluster-manager mode in ovnkube-master mode or not.
...
ovn-master() {
  ...
  cluster_manager_flag=
  if [[ ${ovn_cluster_manager_mode} == "inmaster" ]]; then
    cluster_manager_flag="--init-cluster-manager ${K8S_NODE}"
  fi
  echo "cluster_manager_flag: ${cluster_manager_flag}"
...
# Note: If ovn_master_enable_cluster_manager is set to true, then
# ovnkube-cluster-manager should not be run separately.
ovn-cluster-manager() {
  trap 'kill $(jobs -p); exit 0' TERM
  check_ovn_daemonset_version "3"
