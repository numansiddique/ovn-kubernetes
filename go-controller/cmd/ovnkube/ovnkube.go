...
func runOvnKube(ctx *cli.Context) error {
	...
	if master != "" || local != "" {
		...
		if local != "" {
			klog.Infof("Nums DUDE : Creating local OvnController in node : %q", node)
			...
			klog.Infof("Nums : local OvnController start() returned for : %q", node)
		}
	}
	...
	if node != "" {
		...
		start := time.Now()
		n := ovnnode.NewNode(ovnClientset.KubeClient, nodeWatchFactory, node, stopChan, util.EventRecorder(ovnClientset.KubeClient), local != "")
