...
func createPrimaryIfAddrAnnotation(annotationName string, nodeAnnotation map[string]string, nodeIPNetv4,
...
func CreateNodeTransitSwitchPortAddrAnnotation(nodeAnnotation map[string]string, nodeIPNetv4,
	nodeIPNetv6 *net.IPNet) (map[string]string, error) {
...
// Returns -1 if the 'ovnNodeID' node annotation is not set or if the value is
...
func ParseNetworkIDAnnotation(node *kapi.Node, netName string) (int, error) {
	...
	if !ok {
		return -1, newAnnotationNotSetError("node %q has no %q annotation for network %s", node.Name, ovnNetworkIDs, netName)
	}
...
// GetNodeSubnetAnnotationNetworkNames parses the "k8s.ovn.org/network-ids" annotation
