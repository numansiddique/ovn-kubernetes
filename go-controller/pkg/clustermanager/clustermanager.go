...
func (cm *ClusterManager) Start(nodeName string, wg *sync.WaitGroup, ctx context.Context) error {
	klog.Infof("NUMS : Cluster manager Started.")
...
func (cm *ClusterManager) addNode(node *kapi.Node) error {
	...
	defer func() {
		if err != nil {
			...
			cm.removeNodeId(node.Name, allocateNodeId)
		}
	}()

	allocateNodeId, err = cm.allocateNodeId(node)
	if err != nil {
		return err
	}
	...
	nodeAnnotations[util.OvnNodeId] = strconv.Itoa(allocateNodeId)
