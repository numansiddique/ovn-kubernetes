...
func (oc *DefaultNetworkController) addNode(node *kapi.Node) ([]*net.IPNet, error) {
	...
	// Node gateway router port address(es) is allocated by cluster manager and set in the
	// node's ovnNodeGRLRPAddr annotation. Make sure that it is allocated before proceeding further.
	_, err = util.ParseNodeGatewayRouterLRPAddrs(node)
	if err != nil {
		return nil, fmt.Errorf("failed to get join switch port IP address for node %s: %v", node.Name, err)
	}
