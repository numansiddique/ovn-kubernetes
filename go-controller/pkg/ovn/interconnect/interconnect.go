...
const (
	transitSwitchName      = "transit-switch"
...
func (ic *Controller) updateNode(node *v1.Node) error {
	nodeId := util.GetNodeLocalAZId(node)
	if nodeId == -1 {
...
func (ic *Controller) createLocalAzNodeResources(ni nodeInfo) error {
	...
	ic.addNodeLogicalSwitchPort(types.TransitSwitch, types.TransitSwitchToRouterPrefix+ni.name,
		"router", []string{"router"}, lspOptions)
	return nil
}

func (ic *Controller) addNodeLogicalSwitchPort(logicalSwitchName, portName, portType string, addresses []string, options map[string]string) (string, error) {
	logicalSwitch := nbdb.LogicalSwitch{
		Name: types.TransitSwitch,
		OtherConfig: map[string]string{
			"interconn-ts":      types.TransitSwitch,
...
func (ic *Controller) createRemoteAzNodeResources(ni nodeInfo) error {
	...
	ic.addNodeLogicalSwitchPort(types.TransitSwitch, types.TransitSwitchToRouterPrefix+ni.name, "remote", []string{remotePortAddr}, lspOptions)

	// Set the port binding chassis.
	ic.SetRemotePortBindingChassis(types.TransitSwitchToRouterPrefix+ni.name, ni)
	ic.addRemoteNodeStaticRoutes(ni)
	return nil
}
...
func (ic *Controller) syncNodeResources(ni nodeInfo) {
	if ni.name == ic.localAzName || (ic.localAzName == types.GlobalAz && ni.isNodeGlobalAz) {
		ic.createLocalAzNodeResources(ni)
	} else {
		ic.createRemoteAzNodeResources(ni)
	}
}
