...
func (ic *Controller) removeNode(nodeName string) error {
	if ni, ok := ic.nodes[nodeName]; ok {
		ic.Lock()
		defer ic.Unlock()
		libovsdbops.DeleteNodeChassis(ic.sbClient, ni.name)
...
func (ic *Controller) syncRemoteAzChassis(ni nodeInfo) error {
	...
	defer cancel()
	err := ic.sbClient.Get(ctx, &chassis)
	if err == nil && strings.ToLower(chassis.OtherConfig["is-remote"]) == "true" {
	...
	if err == nil {
		...
		err = ic.sbClient.Get(ctx, &encap_)
		if err == nil && encap_.IP == ni.nodePrimaryIp && encap_.Type == "geneve" {
			//Encap already exists with correct IP and type. Nothing to do.
			return nil
		}
	}
	...
	opModels := []libovsdbops.OperationModel{
		...
		{
			Model:          &chassis,
			ModelPredicate: func(ch *sbdb.Chassis) bool { return ch.Name == ni.chassisID },
			OnModelUpdates: []interface{}{
				&chassis.ExternalIDs,
				&chassis.OtherConfig,
			},
	...
	if _, err := modelSbClient.CreateOrUpdate(opModels...); err != nil {
		return fmt.Errorf("failed to create encaps for chassis %s, error: %v", ni.name, err)
	}
...
func (ic *Controller) syncLocalAzChassis(ni nodeInfo) error {
	...
	// Make sure that the chassis.Externa_ids["is-remote"] is set to "false"
	if strings.ToLower(chassis.ExternalIDs["is-remote"]) == "true" ||
		strings.ToLower(chassis.OtherConfig["is-remote"]) == "true" {
		...
		modelSbClient := libovsdbops.NewModelClient(ic.sbClient)
		if _, err := modelSbClient.CreateOrUpdate(opModels...); err != nil {
			return fmt.Errorf("failed to update chassis %s, error: %v", ni.name, err)
		}
	}
