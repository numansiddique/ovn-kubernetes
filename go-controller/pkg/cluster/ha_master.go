...
func NewHAMasterController(kubeClient kubernetes.Interface, cluster *OvnClusterController,
	nodeName string) *OvnHAMasterController {
...
func (hacluster *OvnHAMasterController) StartHAMasterCluster() error {
	...
	HAClusterNewLeader := func(identity string) {
		...
		if hacluster.nodeName == identity {
			hacluster.ConfigureAsActive(identity)
		} else {
			hacluster.ConfigureAsStandby(identity)
		}
	}
...
//InitHAMasterCluster init.
func (hacluster *OvnHAMasterController) InitHAMasterCluster() error {
	// During initialization, always demote the OVN DBs to backmode mode.
	// After the leader election, the leader will promote the OVN Dbs
	// to become active.
	return hacluster.DemoteOVNDbs(invalidIPAddres, "6641", "6642")
}
...
func (hacluster *OvnHAMasterController) ConfigureAsActive(masterNodeName string) error {
	...
	hacluster.PromoteOVNDbs(strconv.Itoa(int(nbPort)), strconv.Itoa(int(sbPort)))
...
func (hacluster *OvnHAMasterController) ConfigureAsStandby(masterNodeName string) error {
	...
	hacluster.DemoteOVNDbs(masterIP, nbDBPort, sbDBPort)
