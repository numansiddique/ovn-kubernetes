...
func (hacluster *OvnHAMasterController) StartHAMasterCluster() error {
	if hacluster.manageDBServers {
		...
		// to become active.
		err := hacluster.DemoteOVNDbs(invalidIPAddress, ovnNorthDbPort, ovnSouthDbPort)
		if err != nil {
	...
	hacluster.leaderElector, err = leaderelection.NewLeaderElector(lec)
	if err != nil {
		return err
	}
	...
	return hacluster.WatchOvnDbEndpoints()
}
...
func (hacluster *OvnHAMasterController) ConfigureAsActive(masterNodeName string) error {
	if hacluster.manageDBServers {
		...
		err = hacluster.PromoteOVNDbs(ovnNorthDbPort, ovnSouthDbPort)
		if err != nil {
	...
	if ep == nil {
		...
		ovndbEp := v1.Endpoints{
			ObjectMeta: metav1.ObjectMeta{
				...
				Name:        ovnkubeDbEp,
				Annotations: map[string]string{haMasterLeader: hacluster.nodeName},
			},
			Subsets: epSubsets,
		}
		...
		ovndbEp.Annotations[haMasterLeader] = hacluster.nodeName
...
func (hacluster *OvnHAMasterController) ConfigureAsStandby(ep *kapi.Endpoints) error {
	...
	if len(s) != 3 {
		...
		if s[0] != "tcp" || s[1] != masterIPList[0] || s[2] != strconv.Itoa(int(nbDBPort)) {
			activeServerOutOfSync = true
		}
	}
	...
	if !activeServerOutOfSync {
		...
		if len(s) != 3 {
			...
			if s[0] != "tcp" || s[1] != masterIPList[0] || s[2] != strconv.Itoa(int(sbDBPort)) {
				activeServerOutOfSync = true
			}
		}
	}
...
func (hacluster *OvnHAMasterController) PromoteOVNDbs(nbDBPort, sbDBPort int32) error {
...
func (hacluster *OvnHAMasterController) DemoteOVNDbs(masterIP string, nbDBPort, sbDBPort int32) error {
